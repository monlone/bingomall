...
#设置工作目录
WORKDIR $GOPATH/src/bingomall

#添加工作目录
ADD . $GOPATH/src/bingomall

#运行编译
RUN go build .
RUN CGO_ENABLED=0 GOOS=linux go build -a -installsuffix cgo -o bingomall .

#压缩编译后的二进制文件
RUN strip --strip-unneeded bingomall
RUN upx bingomall
...
#从编译器里 copy 二进制文件
COPY --from=builder /go/src/bingomall/bingomall .
COPY --from=builder /go/src/bingomall/view/ ./view/
COPY --from=builder /go/src/bingomall/conf/ ./conf/
COPY --from=builder /go/src/bingomall/logs  ./logs
