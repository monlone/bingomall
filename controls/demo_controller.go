...
// @Router /api/user/demo_list [get]
func DemoList(context *gin.Context) {
	shopId := context.DefaultQuery("shopId", "")
	db := helper.GetDBByName(constant.DBMerchant)
	demoService := service.DemoServiceInstance(repositories.DemoRepositoryInstance(db))
	list := demoService.GetAll(convention.StringToUint64(shopId))
