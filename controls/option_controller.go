...
// @Router /api/user/option_list [get]
func OptionList(context *gin.Context) {
	shopId := context.DefaultQuery("shopId", "")
	db := helper.GetDBByName(constant.DBMerchant)
	optionService := service.OptionServiceInstance(repositories.OptionRepositoryInstance(db))
	list := optionService.GetAll(convention.StringToUint64(shopId))
