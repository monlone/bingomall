...
func (c *Client) enterpriseFillRequestData(params Params) Params {
...
func (c *Client) PostWithCert(url string, params Params) (string, error) {
	str, err := c.postWithCert(url, params)
	return str, err
}
...
func (c *Client) postWithCert(url string, params Params) (string, error) {
	...
	p := c.fillRequestData(params)
	helper.ServiceLogger.Println("postWithCert xml:", strings.NewReader(MapToXml(p)))
	response, err := h.Post(url, bodyType, strings.NewReader(MapToXml(p)))
	if err != nil {
		return "", err
	}
...
func (c *Client) EnterprisePostWithCert(url string, params Params) (string, error) {
	if c.account.certData == nil {
		return "", errors.New("证书数据为空")
	}
	...
	h := &http.Client{Transport: transport}
	p := c.enterpriseFillRequestData(params)
	fmt.Println("MapToXml(p):", MapToXml(p))
	response, err := h.Post(url, bodyType, strings.NewReader(MapToXml(p)))
	if err != nil {
		return "", err
	}
