...
// Pay 支付
func (wechatMini *WechatMiniProgramClient) Pay(charge *common.Charge) (map[string]string, error) {
	var m = make(map[string]string)
	m["appid"] = wechatMini.AppID
	m["mch_id"] = wechatMini.MchID
	...
	sign, err := WechatGenSign(wechatMini.Key, m)
	if err != nil {
...
// 支付到用户的微信账号
func (wechatMini *WechatMiniProgramClient) PayToClient(charge *common.Charge) (map[string]string, error) {
	return WechatCompanyChange(wechatMini.AppID, wechatMini.MchID, wechatMini.Key, wechatMini.httpsClient, charge)
}

// QueryOrder 查询订单
func (wechatMini *WechatMiniProgramClient) QueryOrder(tradeNum string) (common.WeChatQueryResult, error) {
	var m = make(map[string]string)
	m["appid"] = wechatMini.AppID
	m["mch_id"] = wechatMini.MchID
