...
func (aliApp *AliAppClient) Pay(charge *common.Charge) (map[string]string, error) {
	var m = make(map[string]string)
	var bizContent = make(map[string]string)
	m["app_id"] = aliApp.AppID
	...
	bizContent["total_amount"] = AliMoneyFeeToString(charge.MoneyFee)
	...
	m["sign"] = aliApp.GenSign(m)

	return map[string]string{"orderString": aliApp.ToURL(m)}, nil
}

func (aliApp *AliAppClient) PayToClient(charge *common.Charge) (map[string]string, error) {
	return map[string]string{}, errors.New("暂未开发该功能")
}

// 订单查询
func (aliApp *AliAppClient) QueryOrder(outTradeNo string) (common.AliWebAppQueryResult, error) {
	var m = make(map[string]string)
	...
	m["biz_content"] = string(bizContentJson)
	sign := aliApp.GenSign(m)
	m["sign"] = sign

	url := fmt.Sprintf("%s?%s", "https://openapi.alipay.com/gateway.do", aliApp.ToURL(m))

	return GetAlipayApp(url)
}

// GenSign 产生签名
func (aliApp *AliAppClient) GenSign(m map[string]string) string {
	var data []string
	...
	hashByte := s.Sum(nil)
	signByte, err := aliApp.PrivateKey.Sign(rand.Reader, hashByte, crypto.SHA1)
	if err != nil {
		panic(err)
	}
...
// CheckSign 检测签名
func (aliApp *AliAppClient) CheckSign(signData, sign string) {
	...
	hash := s.Sum(nil)
	err = rsa.VerifyPKCS1v15(aliApp.PublicKey, crypto.SHA1, hash, signByte)
	if err != nil {
		panic(err)
	}
}
...
// ToURL
func (aliApp *AliAppClient) ToURL(m map[string]string) string {
	var buf []string
	for k, v := range m {
