...
// 微信企业付款到零钱
func WechatCompanyChange(mchAppid, mchid, key string, conn *HTTPSClient, charge *common.Charge) (map[string]string, error) {
...
//}
func WechatMoneyFeeToString(moneyFee uint64) string {
	str := strconv.FormatInt(int64(moneyFee), 10)
	return str
}

// 支付宝金额转字符串
func AliMoneyFeeToString(moneyFee uint64) string {
	return decimal.NewFromFloat(float64(moneyFee)).Truncate(2).String()
}
