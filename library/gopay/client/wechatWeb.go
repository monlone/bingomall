...
// Pay 支付
func (wechatWeb *WechatWebClient) Pay(charge *common.Charge) (map[string]string, error) {
	var m = make(map[string]string)
	m["appid"] = wechatWeb.AppID
	m["mch_id"] = wechatWeb.MchID
	...
	sign, err := WechatGenSign(wechatWeb.Key, m)
	if err != nil {
...
// 支付到用户的微信账号
func (wechatWeb *WechatWebClient) PayToClient(charge *common.Charge) (map[string]string, error) {
	return WechatCompanyChange(wechatWeb.AppID, wechatWeb.MchID, wechatWeb.Key, wechatWeb.httpsClient, charge)
}

// QueryOrder 查询订单
func (wechatWeb *WechatWebClient) QueryOrder(tradeNum string) (common.WeChatQueryResult, error) {
	var m = make(map[string]string)
	m["appid"] = wechatWeb.AppID
	m["mch_id"] = wechatWeb.MchID
